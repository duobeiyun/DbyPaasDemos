...
QSize QNChatMessage::FontRect(QString str)
{
	...
	//m_iconLeftRect = QRect(iconSpaceW, iconTMPH, m_textWidth, iconWH);
	//m_iconRightRect = QRect(this->width() - iconSpaceW - iconWH, iconTMPH, m_textWidth, iconWH);

	QSize uidsize = getRealString(m_uid);
	int uidhei = uidsize.height() < iconWH ? iconWH : uidsize.height();
	int realIconH = uidhei - m_lineHeight /*- 2 * iconTMPH*/;
	m_iconLeftRect = QRect(0/*iconSpaceW*/, iconTMPH, 60, realIconH);
	m_iconRightRect = QRect(this->width() - iconSpaceW - iconWH, iconTMPH, 60, realIconH);
	...
	int realKuangH = hei - m_lineHeight;
	//realKuangH = realIconH > realKuangH ? realIconH : realKuangH;

	if (size.width() < (m_textWidth + m_spaceWid)) {
		m_kuangLeftRect.setRect(m_sanjiaoLeftRect.x() + m_sanjiaoLeftRect.width(), m_lineHeight / 4 * 3, size.width() - m_spaceWid + 2 * textSpaceRect, realKuangH);
		m_kuangRightRect.setRect(this->width() - size.width() + m_spaceWid - 2 * textSpaceRect - iconWH - iconSpaceW - iconRectW - sanJiaoW,
			m_lineHeight / 4 * 3, size.width() - m_spaceWid + 2 * textSpaceRect, realKuangH);
	}
	else {
		m_kuangLeftRect.setRect(m_sanjiaoLeftRect.x() + m_sanjiaoLeftRect.width(), m_lineHeight / 4 * 3, m_kuangWidth, realKuangH);
		m_kuangRightRect.setRect(iconWH + kuangTMP + iconSpaceW + iconRectW - sanJiaoW, m_lineHeight / 4 * 3, m_kuangWidth, realKuangH);
	}
	...
	return QSize(size.width(), hei > uidhei ? hei : uidhei);
}
...
//        painter.drawRoundedRect(m_iconLeftRect,m_iconLeftRect.width(),m_iconLeftRect.height());
		QPen penText1;
		penText1.setColor(QColor(255, 255, 255));
		painter.setPen(penText1);
		QTextOption option1(Qt::AlignLeft | Qt::AlignVCenter);
		option1.setWrapMode(QTextOption::WrapAtWordBoundaryOrAnywhere);
		painter.setFont(this->font());
		painter.drawText(m_iconLeftRect, m_uid, option1);
...
//        painter.drawRoundedRect(m_iconRightRect,m_iconRightRect.width(),m_iconRightRect.height());
		...
		option1.setWrapMode(QTextOption::WrapAtWordBoundaryOrAnywhere);
		painter.setFont(this->font());
		painter.drawText(m_iconRightRect, m_uid, option1);
...
//void QNChatMessage::showEvent(QShowEvent *event)
//{
