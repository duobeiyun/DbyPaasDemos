...
class DbyVideoView: UIView {
    ...
    func configWith(item: DbyVideoItem) {

        item.videoHostView = self.videoContentView

        self.uid = item.uid
        self.canBeLarge = (!item.isLocal) && (!item.isLarge)
        self.isLarge = item.isLarge
        self.isAudioOn = item.hasAudio
